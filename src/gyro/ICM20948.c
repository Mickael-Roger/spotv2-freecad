...
#define SETTINGS_MODE 1                          // 0 = low power mode, 1 = high performance mode
#define SETTINGS_ENABLE_GYROSCOPE true           // Enables gyroscope output
#define SETTINGS_ENABLE_ACCELEROMETER true       // Enables accelerometer output
#define SETTINGS_ENABLE_MAGNETOMETER true        // Enables magnetometer output // Enables quaternion output
#define SETTINGS_ENABLE_GRAVITY true             // Enables gravity vector output
#define SETTINGS_ENABLE_LINEARACCELERATION true  // Enables linear acceleration output
#define SETTINGS_ENABLE_QUATERNION6 true         // Enables quaternion 6DOF output
#define SETTINGS_ENABLE_QUATERNION9 true         // Enables quaternion 9DOF output
#define SETTINGS_ENABLE_HAR true                 // Enables activity recognition
#define SETTINGS_ENABLE_STEPS true               // Enables step counter
#define SETTINGS_GYROSCOPE_FREQUENCY 1           // Max frequency = 225, min frequency = 1
#define SETTINGS_ACCELEROMETER_FREQUENCY 1       // Max frequency = 225, min frequency = 1
#define SETTINGS_MAGNETOMETER_FREQUENCY 1        // Max frequency = 70, min frequency = 1 
#define SETTINGS_GRAVITY_FREQUENCY 1             // Max frequency = 225, min frequency = 1
#define SETTINGS_LINEARACCELERATION_FREQUENCY 1  // Max frequency = 225, min frequency = 1
#define SETTINGS_QUATERNION6_FREQUENCY 50        // Max frequency = 225, min frequency = 50
#define SETTINGS_QUATERNION9_FREQUENCY 50        // Max frequency = 225, min frequency = 50
#define SETTINGS_HAR_FREQUENCY 50                // Max frequency = 225, min frequency = 50
#define SETTINGS_STEPS_FREQUENCY 50  
...
int i2c_master_write_register(uint8_t address, uint8_t reg, uint32_t len, const uint8_t *data)
{
    ...
    if (write(i2c_file, buffer, len + 1) != len + 1)
    {
...
int i2c_master_read_register(uint8_t address, uint8_t reg, uint32_t len, uint8_t *buff)
{
    ...
    int num_received = read(i2c_file, buff, len);
    if (num_received != len)
    {
...
void init()
{
  ...
  // Set mode
  inv_icm20948_set_lowpower_or_highperformance(&icm_device, SETTINGS_MODE);

  // Set frequency
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_GYROSCOPE), 1000 / SETTINGS_GYROSCOPE_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_ACCELEROMETER), 1000 / SETTINGS_ACCELEROMETER_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_MAGNETOMETER), 1000 / SETTINGS_MAGNETOMETER_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_GAME_ROTATION_VECTOR), 1000 / SETTINGS_QUATERNION6_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_ROTATION_VECTOR), 1000 / SETTINGS_QUATERNION9_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_GRAVITY), 1000 / SETTINGS_GRAVITY_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_LINEAR_ACCELERATION), 1000 / SETTINGS_LINEARACCELERATION_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_BAC), 1000 / SETTINGS_HAR_FREQUENCY);
  rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_STEP_COUNTER), 1000 / SETTINGS_STEPS_FREQUENCY);

  // Enable / disable
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_GYROSCOPE), SETTINGS_ENABLE_GYROSCOPE);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_ACCELEROMETER), SETTINGS_ENABLE_ACCELEROMETER);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_MAGNETOMETER), SETTINGS_ENABLE_MAGNETOMETER);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_GAME_ROTATION_VECTOR), SETTINGS_ENABLE_QUATERNION6);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_ROTATION_VECTOR), SETTINGS_ENABLE_QUATERNION9);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_GRAVITY), SETTINGS_ENABLE_GRAVITY);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_LINEAR_ACCELERATION), SETTINGS_ENABLE_LINEARACCELERATION);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_BAC), SETTINGS_ENABLE_HAR);
  rc = inv_icm20948_enable_sensor(&icm_device, idd_sensortype_conversion(INV_SENSOR_TYPE_STEP_COUNTER), SETTINGS_ENABLE_STEPS);
}
...
void readStepsData(unsigned long* step_count)
{
    *step_count = steps;
...
int main(){
  ...
  while(1){
    ...
    if(quat9DataIsReady() == true){
      ...
      sprintf(msg, "{\"quat_w\":%f, \"quat_x\":%f, \"quat_y\":%f, \"quat_z\":%f}\n", w, x, y, z);
